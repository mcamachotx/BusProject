...
$scope.initialize = function() {
    ...
    if(navigator.geolocation)
    {
        navigator.geolocation.getCurrentPosition(function(position)
        {
            ...
            console.log('Lil panda, lil panda...');
            ...
            var backPath = new google.maps.Polyline({
...
function handleNoGeolocation(errorFlag)
{
    if (errorFlag)
    {
        var content = 'Error: El servicio de geolocalizacion falló.';
    }
    else
    {
 ...
 */
 ...
 function setMarkers(map, locations)
 {
     ...
     for (var i = 0; i < locations.length; i++)
     {
         ...
         var pos = new google.maps.LatLng(parada[1], parada[2]);
...
$scope.calcRoute = function()
{
 ...
 function computeTotalDistance(result)
 {
    ...
    $scope.satelliteButton = function()
     ...
     $scope.trafficButton = function()
     {
         var trafficLayer = new google.maps.TrafficLayer();
         trafficLayer.setMap($scope.map);
     }
    $scope.bicycleButton = function()
     {
         var bicycleLayer = new google.maps.BicyclingLayer();
         bicycleLayer.setMap($scope.map);
     }
    $scope.transitButton = function()
     {
